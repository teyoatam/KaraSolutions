...
Files are saved by default in a folder called _output/data_. These folders are created by the script. You can also give a specific output directory to store collected data.
...
_Note: Your phone must be included to authenticate for the first time. Use the format +\<code>\<number> (e.g., +19876543210). Telegram API will send you a code via Telegram app that you will need to include._
...
- Files of collected channels:
  ...
  - user_exceptions.txt
  - counter.csv
- A new folder: _<channel_name>_ containing
...
- Files of collected channels:
  ...
  - user_exceptions.txt
  - counter.csv
- New folders - based on the number of requested channels: _<channel_name>_ containing
  - A JSON file containing channel's profile metadata
  - A JSON file containing posts from the requested channel
